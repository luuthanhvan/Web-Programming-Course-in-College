		...
		<?php 
			...
			if($result->num_rows > 0){
				echo "<div id='container'>";
					echo "<div id='content'>";
                        echo "<form>";
                            echo "<label id='labelSearchBar'> Tìm kiếm sản phẩm </label>";
                            echo "<input id='searchBar' type='search' onchange='searchProduct(this.value)'/>";
						echo "</form>";
			...
			$connection->close();
        ?>
        <p id="showProductName"></p>
	</body>
	<script> 
			function searchProduct(content){
                // var x = document.getElementById("searchBar");
                // console.log("You are searching for: " + x.value);
                // console.log("You are searching: " + content);
                if(content.length == 0){
                    document.getElementById("showProductName").innerHTML = "";
                    return;
                }
				var xmlHTTP;
				...
				xmlHTTP.onreadystatechange = function(){
					if(xmlHTTP.readyState == 4 && xmlHTTP.status == 200){
                        ...
                        document.getElementById("showProductName").innerHTML = xmlHTTP.responseText;
					}
				}
				xmlHTTP.open("GET", `HandleSearching.php?content=${content}`, true);
				xmlHTTP.send();
			}
	</script>
</html>